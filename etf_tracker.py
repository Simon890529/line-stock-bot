...
def _scrape_capital(etf_code):
    ...
    try:
        resp = requests.get(f"https://www.capitalfund.com.tw/ETFInfo?fundId={etf_code}", headers=_HEADERS, timeout=15)
        holdings = _parse_table(BeautifulSoup(resp.text,"html.parser"))
        if holdings: return holdings
    except Exception as e: logger.warning(f"Capital {etf_code}: {e}")
    return None

def _scrape_president(etf_code):
    try:
        resp = requests.get(f"https://www.puif.com.tw/api/ETF/Holdings?code={etf_code}", headers=_HEADERS, timeout=15)
        data = resp.json()
    ...
    try:
        resp = requests.get(f"https://www.puif.com.tw/products/ETFContent/{etf_code}", headers=_HEADERS, timeout=15)
        holdings = _parse_table(BeautifulSoup(resp.text,"html.parser"))
        if holdings: return holdings
    except Exception as e: logger.warning(f"President {etf_code}: {e}")
    return None

def _scrape_fuh_hwa(etf_code):
    try:
        resp = requests.get(f"https://www.fuhwaetf.com.tw/api/etf/holdings?code={etf_code}", headers=_HEADERS, timeout=15)
        data = resp.json()
        items = data if isinstance(data, list) else data.get("data", [])
        holdings = []
        for item in items:
    ...
    try:
        resp = requests.get(f"https://www.fuhwaetf.com.tw/ETF/{etf_code}", headers=_HEADERS, timeout=15)
        holdings = _parse_table(BeautifulSoup(resp.text,"html.parser"))
        if holdings: return holdings
    except Exception as e: logger.warning(f"Fuh-hwa {etf_code}: {e}")
    return None

def _scrape_allianz(etf_code):
    try:
        resp = requests.get(f"https://www.allianzgi.com.tw/api/etf/portfolio?code={etf_code}", headers=_HEADERS, timeout=15)
        data = resp.json()
        items = data if isinstance(data, list) else data.get("data", data.get("holdings",[]))
        holdings = []
        for item in items:
    ...
    try:
        resp = requests.get(f"https://www.allianzgi.com.tw/tw/etf/{etf_code.lower()}", headers=_HEADERS, timeout=15)
        holdings = _parse_table(BeautifulSoup(resp.text,"html.parser"))
        if holdings: return holdings
    except Exception as e: logger.warning(f"Allianz {etf_code}: {e}")
    return None

def _scrape_yuanta(etf_code):
    try:
        resp = requests.get(f"https://www.yuantaetfs.com/api/get_etf_stockcomponent?strDate=&fundId={etf_code}", headers=_HEADERS, timeout=15)
        data = resp.json()
...
def _scrape_general(etf_code):
    try:
        resp = requests.get(f"https://www.twse.com.tw/zh/ETF/fund/{etf_code}", headers=_HEADERS, timeout=15)
        holdings = _parse_table(BeautifulSoup(resp.text,"html.parser"))
        if holdings: return holdings
    except Exception as e: logger.warning(f"General {etf_code}: {e}")
    return None
