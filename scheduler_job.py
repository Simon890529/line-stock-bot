...
def _push(text: str) -> None:
    ...
    with ApiClient(_line_config) as api_client:
        ...
        for chunk in chunks:
            api.push_message(PushMessageRequest(to=LINE_USER_ID, messages=[TextMessage(text=chunk)]))
...
def start_scheduler() -> None:
    try:
        ...
        scheduler.add_job(run_daily_report, trigger="cron", day_of_week="mon-fri", hour=16, minute=30, id="daily_report")
        scheduler.start()
